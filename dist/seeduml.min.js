...
`).map((c,r)=>`<tspan x="10" dy="${r===0?0:"1.3em"}">${this.escapeXml(c)}</tspan>`).join("")}
        ...
        `}};function _(H){let i=new it,t=new st;try{let e=i.parse(H);return t.render(e)}catch(e){return xt(e)}}function Z(H){let i=new at,t=new lt;try{let e=i.parse(H);return t.render(e)}catch(e){return xt(e)}}function xt(H){let t=(H.message||"Unknown error occurred during parsing").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),e=800,n=100;return`
    ...
    `.trim()}function ht(H){let i=/\b(component|interface|package|node|cloud|database|frame|folder|\[.*?\])\b/.test(H),t=/\b(participant|actor|boundary|control|entity|collections|queue|sequence)\b/.test(H);return i&&!t?Z(H):!i&&t?_(H):/^\[.*?\]/m.test(H)?Z(H):_(H)}function Q(H="pre.seeduml"){if(typeof document>"u")return;document.querySelectorAll(H).forEach(t=>{let e=t.textContent||"",n=ht(e),o=document.createElement("div");o.className="seeduml-diagram",o.innerHTML=n,o.style.display="inline-block",t.parentNode?.replaceChild(o,t)})}function ut(H={}){let{startOnLoad:i=!0,selector:t="pre.seeduml"}=H;i&&(typeof document>"u"||(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{Q(t)}):Q(t)))}typeof window<"u"&&(window.seeduml={renderSequenceDiagram:_,renderComponentDiagram:Z,render:ht,renderAll:Q,initialize:ut});var Lt={renderSequenceDiagram:_,renderComponentDiagram:Z,render:ht,renderAll:Q,initialize:ut};return Mt(Ct);})();
