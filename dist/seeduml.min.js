...
`).map((c,r)=>`<tspan x="10" dy="${r===0?0:"1.3em"}">${_(c)}</tspan>`).join("")}
        ...
        `}normalizeColor(n,t){return n||t}};function J(N){let n=new rt,t=new ct;try{let e=n.parse(N);return t.render(e)}catch(e){return kt(e)}}function tt(N){let n=new ht,t=new ft;try{let e=n.parse(N);return t.render(e)}catch(e){return kt(e)}}function kt(N){let t=(N.message||"Unknown error occurred during parsing").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),e=800,i=100;return`
    ...
    `.trim()}function pt(N){let n=/\b(participant|actor|boundary|control|entity|collections|queue)\b/.test(N),t=/\b(component|package|node|cloud|database|frame|folder)\b/.test(N),e=/^\s*\[[^\]]+\]/m.test(N),i=t||e,o=n;return o&&!i?J(N):i&&!o?tt(N):/\b(alt|else|loop|group|note|opt|par|break|critical|ref)\b/.test(N)?J(N):e?tt(N):J(N)}function et(N="pre.seeduml"){if(typeof document>"u")return;document.querySelectorAll(N).forEach(t=>{let e=t.textContent||"",i=pt(e),o=document.createElement("div");o.className="seeduml-diagram",o.innerHTML=i,o.style.display="inline-block",t.parentNode?.replaceChild(o,t)})}function yt(N={}){let{startOnLoad:n=!0,selector:t="pre.seeduml"}=N;n&&(typeof document>"u"||(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{et(t)}):et(t)))}typeof window<"u"&&(window.seeduml={renderSequenceDiagram:J,renderComponentDiagram:tt,render:pt,renderAll:et,initialize:yt});var At={renderSequenceDiagram:J,renderComponentDiagram:tt,render:pt,renderAll:et,initialize:yt};return Et(Dt);})();
